...
void mainImage( out vec4 fragColor, in vec2 fragCoord, in Image tex )
{
	...
	float red 	=   Texel(	tex, 	vec2(uv.x + xOffset -0.01*rgbOffsetOpt,y)).r+staticVal;
	float green = 	Texel(	tex, 	vec2(uv.x + xOffset,	  y)).g+staticVal;
	float blue 	=	Texel(	tex, 	vec2(uv.x + xOffset +0.01*rgbOffsetOpt,y)).b+staticVal;
