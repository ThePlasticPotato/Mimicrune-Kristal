---@class PsyBallBall : Sprite
---@overload fun(...) : PsyBallBall
local PsyBallBall, super = Class(Sprite)

function PsyBallBall:init(green, x, y, tx, ty, after)
    super.init(self, green and "effects/psyball/beam_green" or "effects/psyball/beam", x, y)
...
function PsyBallBall:update()
    ...
    if Utils.dist(self.x, self.y, self.target_x, self.target_y) <= 40 then
        ...
        for i = 1, 8 do
            local burst = PsyBurst(self.green, self.target_x, self.target_y, math.rad(45 + ((i - 1) * 90)), i > 4)
    ...
    if self.afterimg_timer >= 1 then
        ...
        local sprite = Sprite(self.green and "effects/psyball/beam_green" or "effects/psyball/beam", self.x, self.y)
