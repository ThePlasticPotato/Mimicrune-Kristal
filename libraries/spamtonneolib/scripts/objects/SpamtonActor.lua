...
function Spamton:init(actor)
    ...
    self.body = SpamtonPart("enemies/lanturn/idle", 0, 0, 0, 0.33)
    self.body.id = "body"
    self.body.layer = 8
    self.body.swing_range = {math.rad(-7.5),math.rad(7.5)}
    self:addChild(self.body)

    self.leg_l = SpamtonPart("placeholder", 50, 35, 0.5, 0)
    self.leg_l.id = "leg_l"
    self.leg_l.layer = -10
    self:addChild(self.leg_l)

    self.leg_r = SpamtonPart("placeholder", 70, 15, 0.5, 0)
    self.leg_r.id = "leg_r"
    self.leg_r.layer = -10
    self:addChild(self.leg_r)

    self.arm_l = SpamtonPart("placeholder", 100, 25, 0, 0.5)
    self.arm_l.id = "arm_l"
    self.arm_l.layer = -12
    self:addChild(self.arm_l)

    self.arm_r = SpamtonPart("placeholder", 110, 45, 0, 1)
    self.arm_r.id = "arm_r"
    self.arm_r.layer = -4
    self:addChild(self.arm_r)

    self.wing_l = SpamtonPart("placeholder", 100, 0, 1, 0.5)
    self.wing_l.id = "wing_l"
    self.wing_l.layer = -15
    self:addChild(self.wing_l)

    self.wing_r = SpamtonPart("placeholder", 70, -10, 0, 0.5)
