...
local midiEvent = {
  [0x80] = function(stream, callback, channel, fb)
    local key = fb:byte()
    local velocity = stream:read(1):byte()
  ...
  [0xE0] = function(stream, callback, channel, fb)
    local lsb = fb:byte()
    local msb = stream:read(1):byte()
    local pitch14 = bor(lsb, lshift(msb, 7)) -- 0..16383
    callback("pitch", channel, (pitch14 / 0x2000) - 1) -- -1..~+0.9999
    return 2
  end
}
...
local metaEvents = {
  ...
  [0x51] = function(data, callback)
    ...
    local rawTempo = b1 * 65536 + b2 * 256 + b3
  ...
  [0x59] = function(data, callback)
    local count, minor = data:byte(1, 2)
    if count >= 128 then count = count - 256 end
...
local function readChunkInfo(stream)
  ...
  local b1, b2, b3, b4 = chunkInfo:byte(5, 8)
...
local function readHeader(stream, callback, chunkLength)
  ...
  local b1, b2, b3, b4, b5, b6 = header:byte(1, 6)
  local format = b1 * 256 + b2
  local tracks = b3 * 256 + b4
  local division = b5 * 256 + b6
...
local function processEvent(stream, callback, runningStatus)
  ...
  if status >= 0x80 and status < 0xF0 then
    local eventType = band(status, 0xF0)
    local channel = band(status, 0x0F) + 1
    length = length + midiEvent[eventType](stream, callback, channel, firstByte)
