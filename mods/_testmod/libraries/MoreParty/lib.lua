...
function Lib:init()
    ...
    HookSystem.hook(ActionBoxDisplay, "draw", function(orig, self)
        ...
        local color_to_use = {self.actbox.battler.chara:getColor()}
        if (self.actbox.rainbow) then
            color_to_use = {self.actbox:getRainbowColor()}
        end
        
        local x = 212
        
        if Game.battle.current_selecting == self.actbox.index then
            Draw.setColor(color_to_use)
        else
        ...
        if not Kristal.getLibConfig("moreparty", "classic_mode") and #Game.party < 4 then
    ...
    HookSystem.hook(ActionBox, "drawActionBox", function(orig, self)
        ...
        local color_to_use = {self.battler.chara:getColor()}
        if (self.rainbow) then
            color_to_use = {self:getRainbowColor()}
        end
        local x = 212
    ...
    HookSystem.hook(BattleUI, "drawActionStrip", function(orig, self)
        ...
        if #Game.battle.party <= (Kristal.getLibConfig("moreparty", "classic_mode") and 3 or 3) then return end
        
        -- Draw the top line of the action strip
        Draw.setColor(PALETTE["action_strip"])
        love.graphics.rectangle("fill", 0, Game:getConfig("oldUIPositions") and -35 or -37, 640, Game:getConfig("oldUIPositions") and 3 or 4)
    ...
    if not Mod.libs["ExpandedAttackLib"] then -- Compatibility with 'ExpandedAttackLib' Library.
        HookSystem.hook(AttackBox, "draw", function(orig, self)
            ...
            local target_color = self.battler.chara.rainbow_battlebox and {self:getRapidRainbowColor()} or {self.battler.chara:getAttackBarColor()}
            local box_color = self.battler.chara.rainbow_battlebox and {self:getRainbowColor()} or {self.battler.chara:getAttackBoxColor()}
            ...
            local height = #Game.battle.party > 2 and math.floor(104 / #Game.battle.party) or 36
    ...
    if not Mod.libs["ExpandedAttackLib"] then -- Compatibility with 'ExpandedAttackLib' Library.
        HookSystem.hook(AttackBox, "init", function(orig, self, battler, offset, index, x, y)
            ...
            self.head_sprite:setOrigin(0.5, 0.75 + (2 * (#Game.battle.party - 2) * 0.075))
            self.press_sprite:setOrigin(0, (#Game.battle.party - 2) * 0.025)
            self.head_sprite:setScale(1 - ((#Game.battle.party - 2) * 0.125))
        end)
    end
    ...
    HookSystem.hook(BattleUI, "init", function(orig, self)
        if #Game.battle.party <= (Kristal.getLibConfig("moreparty", "classic_mode") and 3 or 3) then
            orig(self)
        else
            ...
            if #Game.battle.party == 3 then
                size_offset = 0
                box_gap = 0
            elseif (#Game.battle.party == 2) or (#Game.battle.party > 3) then
                size_offset = 108
                box_gap = 1
            ...
            for index,battler in ipairs(Game.battle.party) do
                local action_box = ActionBox(size_offset+ (((index - 1) % 2)) * (213 + box_gap), 0, index, battler)
        ...
        local x = 0
        local realW = ((640 - 1) / (Kristal.getLibConfig("moreparty", "classic_mode") and 3 or 2)) -- SCREEN_WIDTH
        
        local e = 0
        
        for k,v in ipairs(self.action_boxes) do
            if k > (Kristal.getLibConfig("moreparty", "classic_mode") and 3 or 2) then
        ...
        if #Game.battle.party > (Kristal.getLibConfig("moreparty", "classic_mode") and 3 or 2) then
            for k,v in ipairs(self.action_boxes) do
    ...
    HookSystem.hook(BattleUI, "getTransitionBounds", function(orig, self)
        if #Game.battle.party <= (Kristal.getLibConfig("moreparty", "classic_mode") and 3 or 3) then return orig(self) end
    ...
    HookSystem.hook(Encounter, "getPartyPosition", function(orig, self, index)
        ...
        local reset = 0
        local middle = 0
        local classic = (Kristal.getLibConfig("moreparty", "classic_mode") and 3 or 2)
        ...
        x = 80 + column
        y = (((not Kristal.getLibConfig("moreparty", "classic_mode") and #Game.battle.party <= 3) and 120 or 50) / classic) + ((SCREEN_HEIGHT * 0.5) / classic) * (index - 1 - reset) + middle
    ...
    HookSystem.hook(DarkMenuPartySelect, "init", function(orig, self, x, y)
        orig(self, x, y)
    ...
    HookSystem.hook(ActionBox, "draw", function(orig, self)
        if #Game.battle.party <= 3 then orig(self) return end 
   ...
   if not Mod.libs["better_battles"] then -- Compatibility with 'better_battles' Library.
        HookSystem.hook(ActionButton, "select", function(orig, self)
            ...
            if self.type == "act" and not Kristal.getLibConfig("moreparty", "classic_mode") and #Game.battle.party > 4 and self.battler.chara:hasSpells() then
        ...
        HookSystem.hook(ActionBox, "createButtons", function(orig, self)
            ...
            if not self.battler.chara:hasSpells() or self.battler.chara:hasAct() and not Kristal.getLibConfig("moreparty", "classic_mode") and #Game.battle.party > 4 and self.battler.chara:hasSpells() then TableUtils.removeValue(btn_types, "magic") end
    ...
    if Mod.libs["magical-glass"] then -- Compatibility with 'magical-glass' Library.
        ...
        HookSystem.hook(LightStatusDisplay, "drawStatusStrip", function(orig, self)
                ...
                for index,battler in ipairs(Game.battle.party) do
                    if #Game.battle.party == 4 and not Kristal.getLibConfig("moreparty", "classic_mode") then
                        ...
                        if current > 0 then
                            ...
                            love.graphics.rectangle("fill", x + 62, y + (small and 3 or 0), math.ceil((MathUtils.clamp(current - karma, 0, max) / max) * (small and 12 or 26)) * 1.2 + 1 - (karma_mode and 1 or 0), small and 14 or 21)
                        end
                        ...
                        if current > 0 then
                            ...
                            love.graphics.setColor(Game:isLight() and MG_PALETTE["player_health"] or {battler.chara:getColor()})
                            love.graphics.rectangle("fill", x + 62, y - 2, math.ceil((MathUtils.clamp(current - karma, 0, max) / max) * (small and 12 or 26)) * 1.2 + 1 - (karma_mode and 1 or 0), 10)
                        end
        ...
        HookSystem.hook(LightHealthBar, "init", function(orig, self)
             orig(self)
             if #Game.party <= 3 then return end
        ...
        HookSystem.hook(LightHealthBar, "draw", function(orig, self)
            if #Game.party <= 3 then orig(self) return end
    ...
    HookSystem.hook(ActionBox, "createButtons", function(orig, self)
        orig(self)
        if #Game.battle.party > 4 and Kristal.getLibConfig("moreparty", "classic_mode") then
    ...
    HookSystem.hook(ActionBox, "update", function(orig, self)
        ...
        if #Game.battle.party <= 5 or Kristal.getLibConfig("moreparty", "classic_mode") then return end
