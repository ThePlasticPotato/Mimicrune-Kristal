...
function ModlandFileSelect:update()
    ...
    if (self.state == "SELECT") then
        local file_selected, index = self:getSelectedFile()
        ...
        if (Game.stage.weather_type ~= "clear") then Game.stage:setWeather("clear", false) end
...
function ModlandFileSelect:draw()
    ...
    if self.state == "SELECT" or self.state == "TRANSITIONING" then
        ...
        if not self.chapter_select then
            ...
            setColor(3, 5)
            Draw.printShadow(self:gasterize "Quit", self.bottom_row_heart[3] + 28, 380 + 40)
        end
