...
function HealingParticles:update()
    ...
    if self.particle_timer >= 0.25 then
        self.particle_timer = 0
        local radius = MathUtils.random() * 16 + 5
        local plusRand = MathUtils.random() > 0.5
        table.insert(self.particles, {radius = radius, x = MathUtils.random() * SCREEN_WIDTH, y = SCREEN_HEIGHT + radius, max_radius = radius, speed = MathUtils.random() * 0.5 + 0.5, has_plus = plusRand})
    end
end
