...
Currently based on version 1.92.0 (docking branch) of Dear ImGui and LÖVE 11.5.
...
- Some functions expect a `char` buffer as input. A buffer of, for example, size 10 can be created using `buffer = ffi.new("char[10]","")` (note that size 10 includes space for a null character at the end, so it can actually only fit 9 characters in). The buffer can be then used like this: `imgui.InputText("Some field", buffer, 10)`. To convert the buffer to a Lua string use `ffi.string(buffer)`.
...
See the example above to figure out how to use them.
...
### Changing fonts
If you change or add fonts using the appropriate Dear ImGui functions, with some caveats about ownership of the font data. For example:
```lua
local ffi = require "ffi"
...
### Images
You can add LÖVE images to ImGui with `imgui.Image`, `imgui.ImageButton`, etc. These functions expect an `ImTextureRef` input, which can be obtained by passing a LÖVE `Texture` to the `imgui.love.TextureRef` functions.
```lua
...
love.draw = function()
    if imgui.Begin("test window") then
        imgui.Image(imgui.love.TextureRef(img), size)
    end
    imgui.End()
...
```
The same applies to the wrappers of other functions taking "ImTextureRef" as an argument.
